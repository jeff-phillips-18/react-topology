...
describe('createAggregateEdges', () => {
  describe('default / collapsedGroups', () => {
    ...
    it('marks aggregates bidirectional when directions differ', () => {
      ...
      const result = aggregate(edges, nodes);
      expect(aggregates(result)[0].data.bidirectional).toBe(true);
    });
  ...
  describe('edge cases', () => {
    ...
    it('returns edges unchanged when nodes are missing', () => {
      const edges = [edge('n1', 'n2')];
      expect(createAggregateEdges('aggregate-edge', edges, undefined)).toBe(edges);
    });
  });
});
